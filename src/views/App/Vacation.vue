    ...
    >
      ...
      <template v-slot:top="item">
        ...
        <div class="d-flex flex-row mt-2 align-end">
          ...
          />
          <v-spacer/>
          <v-btn v-if="!schedule.isActive" class="ma-2" color="primary" text @click="onAddVocation">
            добавить отпуск
          </v-btn>
        </div>
      </template>
...
export default {
  ...
  mixins: [dataMethods, VacationMethods, posts, getShortDayLabel, templateFile,
    lowerCase, normalizeDate, vacationStatuses, appReady, getShortUserNameByUID],
  data: () => ({
    schedule: null,
    user: null,
  ...
  methods: {
    ...
    async downloadApplication(id) {
      ...
      await this.vacations[id].downloadApplication()
    ...
    deleteAlertSubmit() {
      this.vacations[this.deleteId].delete().then(
